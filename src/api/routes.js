...
export function registerRoutes(route) {
  ...
  route('POST /api/ai/image', async (req, res, { readJsonBody }) => {
    ...
    try {
      const r = await execute('image', { prompt: body.prompt, refImages: body.refImages || [], aspect: body.aspect, editFunction: body.editFunction, editStrength: body.editStrength });
  ...
  route('POST /api/artist/:id/portrait', async (req, res, { params, readJsonBody }) => {
    ...
    try {
      ...
      const built = buildPortraitPrompt(artist, body.stylePrompt, { overrideLook: look });
      const override = String(body.promptOverride || '').trim();   // 用户改过的完整提示词：原样出图，不再智能扩写
      const prompt = override || built.prompt;
      const negative = built.negative;
      // previewOnly：只返回将要使用的提示词，不出图（让用户先看、可调整再确认生成，不花钱）
      if (body.previewOnly) return json(res, { prompt, negative });
      // 指定新外形 / 用了自定义提示词时关掉智能扩写，确保如实采纳；外形里的否定项并入负向词
      const r = await execute('image', { prompt, refImages: [], aspect: '3:4', promptExtend: !look && !override, negativePrompt: ['全身, 全身照, 站姿全身, 远景, 大长腿, 露出双腿, 半身以下', negative].filter(Boolean).join(', ') });
  ...
  route('POST /api/artist/:id/interview2/:sid/looks', async (req, res, { params, readJsonBody }) => {
    ...
    res.writeHead(200, { 'Content-Type': 'text/event-stream; charset=utf-8', 'Cache-Control': 'no-store', Connection: 'keep-alive' });
    const send = (event, data) => res.write(`event: ${event}\ndata: ${JSON.stringify(data)}\n\n`);
    // 直接基于真实照片做指令编辑（qwen-image-edit），原地保留本人五官长相，只改场景/服装/构图——
    // 不再走「照片→VL 文字→重绘」的失真中转。指令对男女通用（模型会保留照片里的性别）。
    const EDIT_INSTRUCTION = '把照片中的人物变成坐在专业新闻演播室主播台前的新闻主播：换上深色职业正装，背景改为虚化的新闻演播室大屏与演播台灯光，半身居中、正脸面向镜头的特写构图，神情专业自信。务必完整保留这个人的五官长相、脸型、发型、发色、肤色、性别，仍是同一个人，不要改变其相貌。';
    try {
      const mk = async (name, label, srcUrl, pBase) => {
        const ref = generatedUrlToDataUrl(GENERATED_DIR, srcUrl);
        if (!ref) throw new Error('形象底图读取失败');
        send('stage', { progress: pBase, msg: `生成${name}主播形象` });
        const r = await execute('image-edit', { image: ref, prompt: EDIT_INSTRUCTION });
        const u = r.files?.[0]?.url; if (!u) throw new Error('出图失败');
        addAssets(params.id, [{ type: 'photo', url: u, prompt: EDIT_INSTRUCTION, title: label }]);
        return u;
      };
      const hostLook = await mk('主持人', '主持人主播形象', hostSrc, 20);
      const guestLook = await mk('嘉宾', '嘉宾主播形象', guestSrc, 60);
