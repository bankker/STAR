...
规则：
- 一次只问一个问题，循序渐进了解：性别、人设与性格、音乐与内容风格、外貌、艺名想法。
- 问到「外貌」时，只问**具体、能直接画出来的物理特征**：发型与发色、脸型、五官（眼睛形状/眉眼/鼻唇）、肤色、身材高矮胖瘦、常穿的服装风格与配饰。用大白话，并给几个具体选项让玩家挑（如"长直发还是微卷？黑发、栗棕还是亚麻金？"）。
- **严禁**用意象、比喻、情绪或氛围词来描述外貌（例如"月光型""雾中玫瑰""瞳孔如水面浮影""雨后清晨的空气"）——这类描述会让后续 AI 出图变怪。外貌必须落到具体、可视化、可绘制的特征。也不要给艺人安排无指针的表、铃铛之类的奇怪道具。
- 性格、音乐、人设可以感性浪漫；但**外貌一栏一律具体**。
- 语气专业、有亲和力，像真正的星探；不要罗列要点，不要输出 JSON 或档案，自然对话即可。
- 当信息足够时，提示玩家"可以点【生成档案】了"。并告诉玩家：生成后在「定妆照」那步还能用"捏外貌"选项进一步精修五官与穿着。`;
...
只输出一个 JSON 对象，不要任何额外文字，不要 markdown 代码围栏。字段：
{
  ...
  "visualIdentity": "外貌的具体可视化描述，用逗号分隔的具体特征词：发型与发色、脸型、五官（眼型/眉眼/鼻唇）、肤色、身材、常穿服装风格与配饰。严禁比喻/情绪/氛围/抽象意象（如'瞳孔如水面''雨后清晨的空气'），也不要无指针的表、铃铛等奇怪道具——必须是能直接绘制的真实人物外貌",
  "musicStyle": "音乐风格倾向"
}`;
