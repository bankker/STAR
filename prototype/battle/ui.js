...
function renderMapDetail(n) {
  ...
  const eUrl = n.enemy && G.ships ? G.ships[enemyShipId({ name: n.enemy.name, fac: n.fac, maxHp: n.enemy.hp })] : '';
  const eThumb = !n.enemy ? '' : (eUrl
    ? `<div style="width:136px;height:100px;flex:none;background-image:url('${esc(eUrl)}');background-size:contain;background-repeat:no-repeat;background-position:center;filter:drop-shadow(0 0 14px ${fc}66)"></div>`
    : `<div style="width:136px;height:100px;flex:none;filter:drop-shadow(0 0 14px ${fc}66)">${warshipSVG({ boss: n.type === 'boss' })}</div>`);
  const enemyBlock = n.enemy ? `<div><div style="font-size:11px;letter-spacing:3px;color:#7a93a8;margin-bottom:9px">敌军预览</div><div style="display:flex;align-items:center;gap:14px;padding:14px;background:rgba(8,18,30,.7);border:1px solid ${fc}55;border-radius:8px">${eThumb}<div style="flex:1"><div style="font-weight:700;font-size:15px;color:#ffd9c8">${esc(n.enemy.name)}</div><div style="display:flex;gap:14px;margin-top:6px;font-size:12px"><span style="color:#ff8a6a">HP <span style="font-family:Oxanium;font-weight:800;font-size:15px">${n.enemy.hp}</span></span><span style="color:#9fb6c6">护甲 <span style="font-family:Oxanium;font-weight:800;font-size:15px;color:#cfe6ff">${n.enemy.armor}</span></span></div><div style="margin-top:8px;display:flex;align-items:center;gap:7px;padding:5px 10px;background:rgba(40,16,16,.7);border:1px solid rgba(255,90,60,.4);border-radius:6px;width:fit-content"><span style="width:15px;height:15px;background-image:url(&quot;${Q(nodeIcon('combat', '#ff7a5a'))}&quot;);background-size:contain;background-repeat:no-repeat"></span><span style="font-size:11px;color:#ffb0a0">下回合意图 · ${esc(n.enemy.intent)}</span></div></div></div></div>` : '';
